...
// Check channel-specific dist dir first, then root dist/
const _channelName = process.env.CHANNEL_PM2_NAME?.replace("banner-daemon-", "")
  ?? process.env.CHANNEL_CONFIG?.match(/channels\/([^/]+)\/config/)?.[1]
  ?? null;
const _distDir  = _channelName ? `dist/${_channelName}` : "dist";
const _v2File   = resolve(ROOT, `${_distDir}/banner-v2.png`);
const _v1File   = resolve(ROOT, "dist/banner.png");
const BANNER_FILE   = resolve(process.env.BANNER_FILE
  ?? (existsSync(_v2File) ? `${_distDir}/banner-v2.png` : "dist/banner.png"));
